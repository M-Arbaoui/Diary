...
export default function Journal() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-3 md:px-6 lg:px-10 py-6 md:py-10 lg:py-14 pb-safe">
      <header className="mb-8 md:mb-10 animate-fade-up flex items-end justify-between gap-4 flex-wrap">
        <div className="flex-1 min-w-0">
          <p className="text-[11px] uppercase tracking-[0.22em] text-ink-muted">Your journal</p>
          <h1 className="font-serif text-3xl md:text-4xl lg:text-5xl text-ink mt-2">
        ...
        <Button
          ...
          className="rounded-full bg-foreground text-background hover:bg-foreground/90 h-12 md:h-11 px-6 flex-shrink-0 font-medium text-sm md:text-base"
          title="Create new entry (Cmd/Ctrl + N)"
        >
          <Plus className="h-4 w-4 mr-2" /> <span className="hidden sm:inline">New entry</span><span className="sm:hidden">New</span>
        </Button>
      </header>
...
function EntryEditor({
  ...
  return (
    <div className="max-w-3xl mx-auto px-3 md:px-6 lg:px-10 py-6 md:py-8 lg:py-12 animate-fade-in pb-safe">
      <div className="flex items-center justify-between mb-8 gap-2">
        <Button
          variant="ghost"
          onClick={onClose}
          className="rounded-full hover:bg-card text-ink-muted -ml-2 md:-ml-3 h-12 md:h-10 text-sm md:text-base"
        >
          <ArrowLeft className="h-4 w-4 mr-2" /> Back
        </Button>

        <div className="flex items-center gap-2 text-xs text-ink-muted h-8 flex-shrink-0">
          {status === "saving" && (
      ...
      <div className="mt-12 flex flex-col gap-3 md:flex-row items-stretch justify-between border-t border-border/60 pt-6">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-2">
          {currentId && (
            <Button
              ...
              className="rounded-full text-destructive border border-destructive/15 hover:bg-destructive/10 hover:text-destructive h-12 px-4 sm:px-5"
            >
          ...
          <Button
            onClick={handleDone}
            className="rounded-full bg-foreground text-background hover:bg-foreground/90 active:bg-foreground/95 h-12 px-5 font-medium"
